 ...
 * exceed container boundaries.
 */
TEST(TestASN1, OverflowGuard_Next)
{
    ASN1Reader reader;

    // Craft a DER blob where an element inside a SEQUENCE claims a length
    // that exceeds the container boundary. DecodeHead validates against mBufEnd
    // (the entire buffer), not mContainerEnd, so it will succeed. The new bounds
    // check in Next() catches this: after parsing the oversized element, Next()
    // verifies that mHeadLen + ValueLen does not exceed the remaining container
    // space before advancing mElemStart.
    //
    // Layout:
    //   SEQUENCE (tag=0x30, len=3): container has only 3 bytes
    //     INTEGER (tag=0x02, len=4): claims 4 value bytes, extends past container
    //   <extra buffer bytes so DecodeHead does not fail on mBufEnd check>
    //
    // After first Next() inside SEQUENCE: mHeadLen=2, ValueLen=4
    // Second Next(): bounds check => (mContainerEnd - mElemStart)=3 < (2+4)=6 => ASN1_END

    // clang-format off
    static const uint8_t kOverflowTestDER[] = {
        0x30, 0x03,                         // SEQUENCE, length = 3
        0x02, 0x04, 0x01, 0x02, 0x03, 0x04, // INTEGER, length = 4 (exceeds container)
        0x00, 0x00                           // padding so buffer is large enough
    };
    // clang-format on

    reader.Init(kOverflowTestDER, sizeof(kOverflowTestDER));

    // Parse the outer SEQUENCE
    CHIP_ERROR err = reader.Next();
    EXPECT_EQ(err, CHIP_NO_ERROR);
    EXPECT_EQ(reader.GetClass(), kASN1TagClass_Universal);
    EXPECT_EQ(reader.GetTag(), kASN1UniversalTag_Sequence);

    // Enter the SEQUENCE (mContainerEnd = buf+2+3 = buf+5)
    err = reader.EnterConstructedType();
    EXPECT_EQ(err, CHIP_NO_ERROR);

    // First Next(): DecodeHead parses INTEGER with ValueLen=4 (succeeds because
    // mBufEnd is large enough, even though it exceeds the 3-byte container)
    err = reader.Next();
    EXPECT_EQ(err, CHIP_NO_ERROR);
    EXPECT_EQ(reader.GetTag(), kASN1UniversalTag_Integer);
    EXPECT_EQ(reader.GetValueLen(), 4u);

    // Second Next(): the bounds check catches that mHeadLen(2) + ValueLen(4) = 6
    // exceeds remaining container space of 3 bytes
    err = reader.Next();
    EXPECT_EQ(err, ASN1_END);
}

TEST(TestASN1, OverflowGuard_ExitContainer)
{
    ASN1Reader reader;

    // Test that ExitContainer works correctly with the overflow guard.
    // Use a nested SEQUENCE structure and verify that after exiting the
    // inner SEQUENCE, parsing continues correctly in the outer container.

    // clang-format off
    static const uint8_t kExitTestDER[] = {
        0x30, 0x06,             // Outer SEQUENCE, length = 6
        0x30, 0x03,             //   Inner SEQUENCE, length = 3
        0x02, 0x01, 0x42,       //     INTEGER, length = 1, value = 0x42
        0x02, 0x01, 0x07        //   INTEGER, length = 1, value = 0x07
    };
    // clang-format on

    reader.Init(kExitTestDER, sizeof(kExitTestDER));
    ...
    // Parse the INTEGER inside inner SEQUENCE
    err = reader.Next();
    EXPECT_EQ(err, CHIP_NO_ERROR);
    EXPECT_EQ(reader.GetTag(), kASN1UniversalTag_Integer);
